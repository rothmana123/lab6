 ...
 *
 * Implements a C memory allocator with a FIFO free list cache. 
 * Users can setALLOC_THRESH to determine how many free blocks are cached in the free list.
 */
...
static size_t alloc_threshold = 100; // default
...
void *malloc_name(size_t size, const char *name)
{
    ...
    block->name[BLOCK_NAME_LEN - 1] = '\0'; // null-terminate
...
void *malloc(size_t size)
{
    ...
    if (block != NULL)
    {
        return block + 1; // return usable memory after metadata
    }

    // No reusable block found — fallback to mmap
...
void free(void *ptr)
{
    ...
    add_to_free_list(block); // insert the newly freed block
...
void *realloc(void *ptr, size_t size)
{
    ...
    // basically, if the current block is already big enough, return it
    if (old_size >= size)
    {
        return ptr;
    }
